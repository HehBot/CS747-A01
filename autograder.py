...
def grade_task1(tc_path, algo):
    ...
    if algo == "thompson" or algo == "all":
        ...
        scores["Thompson Sampling"] = (
            1 if regrets["Thompson Sampling"] <= tc.thompson else 0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The task to run. Valid values are: 1, 2, 3, all",
    )
    parser.add_argument(
        "--algo",
        type=str,
        required=False,
        help="The algo to run (for task 1 only). Valid values are: ucb, kl_ucb, thompson, all",
    )
    ...
    if args.task == "1" or args.task == "all":
        ...
        for i in range(1, 4):
            ...
            for algo, score in scores.items():
                print(
                    "{:18}: {}. Regret: {:.2f}".format(
                        algo, pass_fail[int(score * 2)], regrets[algo]
                    )
                )
            print("")
    ...
    if args.task == "3" or args.task == "all":
        ...
        for i in range(1, 4):
            ...
            print(
                "Faulty Bandit Algorithm: {}. Reward: {:.2f}".format(
                    pass_fail[int(score * 2)], reward
                )
            )
            print("")
    ...
    if args.task == "4" or args.task == "all":
        ...
        for i in range(1, 4):
            ...
            print(
                "MultiBandit Algorithm: {}. Reward: {:.2f}".format(
                    pass_fail[int(score * 2)], reward
                )
            )
            print("")
